...
chrome.storage.local.get(["user_id"], local => {
    ...
    */
    ...
    .then(({data:res}) => {
        ...
        let g = t[0]
        console.error(g)
        g.forEach(({username : profAt, name : username, profile_image_url : profPic, url}) => {
            blockedUsersDiv.insertAdjacentHTML('beforeend',createProfileNode(username, profAt, profPic,url))
        })
    })
    axios.get(getMutedUsersUrl)
    .then(({data:res}) => {
        console.error(res)
        const { data:t } = res
        console.error(t)
